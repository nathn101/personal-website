...
class CardBox {
    ...
    constructor(DOM_el) {
        ...
        if ( this.DOM.tags ) {
            this.DOM.tags.dataset.splitting = '';
        }
        ...
        if (this.DOM.tags) {
            this.DOM.tagsChars = this.DOM.tags.querySelectorAll('.char');
        }
    }
...
export class Card1 {
    ...
    get cardBoxTagsChars() {
        return this.cardBoxesArr.map(box => box.DOM.tagsChars)
    ...
    enter() {
        if (this.DOM.img === null) {
            console.log('this.DOM.img is null');
            return;
        }
